...
import { TermFrequency } from "../../common/v1/term_frequency.pb";
...
export const UpdateRequest = {
  ...
  _writeMessageJSON: function (
    ...
    if (msg.answers?.length) {
      json.answers = msg.answers.map(InputValue._writeMessageJSON);
    }
    return json;
  },
  ...
  _readMessage: function (
    ...
    while (reader.nextField()) {
      ...
      switch (field) {
        ...
        case 3: {
          const m = InputValue.initialize();
          reader.readMessage(m, InputValue._readMessage);
          msg.answers.push(m);
          break;
        }
        default: {
  ...
  _readMessageJSON: function (msg: UpdateRequest, json: any): UpdateRequest {
    ...
    const _answers = json.answers;
    if (_answers) {
      for (const item of _answers) {
        const m = InputValue.initialize();
        InputValue._readMessageJSON(m, item);
        msg.answers.push(m);
      }
    }
    return msg;
...
export const PreviousResponse = {
  ...
  initialize: function (): PreviousResponse {
    return {
      progress: Progress.initialize(),
      step: Step.initialize(),
      termFrequency: TermFrequencyFromInt(0),
    };
  },
  ...
  _writeMessage: function (
    ...
    if (msg.termFrequency && TermFrequencyToInt(msg.termFrequency)) {
      writer.writeEnum(3, TermFrequencyToInt(msg.termFrequency));
    }
    return writer;
  },
  ...
  _writeMessageJSON: function (
    ...
    if (msg.termFrequency && TermFrequencyToInt(msg.termFrequency)) {
      json.termFrequency = msg.termFrequency;
    }
    return json;
  },
  ...
  _readMessage: function (
    ...
    while (reader.nextField()) {
      ...
      switch (field) {
        ...
        case 3: {
          msg.termFrequency = TermFrequencyFromInt(reader.readEnum());
          break;
        }
        default: {
          reader.skipField();
  ...
  _readMessageJSON: function (
    ...
    const _termFrequency = json.termFrequency ?? json.term_frequency;
    if (_termFrequency) {
      msg.termFrequency = _termFrequency;
    }
    return msg;
  },
};
...
export const Step = {
  ...
  _writeMessage: function (
    ...
    if (msg.inputDescriptors?.length) {
      writer.writeRepeatedMessage(
        3,
        msg.inputDescriptors as any,
        InputDescriptor._writeMessage
      );
    }
    if (msg.answers?.length) {
      writer.writeRepeatedMessage(
  ...
  _writeMessageJSON: function (msg: Partial<Step>): Record<string, unknown> {
    ...
    if (msg.inputDescriptors?.length) {
      json.inputDescriptors = msg.inputDescriptors.map(
        InputDescriptor._writeMessageJSON
      );
    }
    if (msg.answers?.length) {
      json.answers = msg.answers.map(InputValue._writeMessageJSON);
  ...
  _readMessage: function (msg: Step, reader: BinaryReader): Step {
    while (reader.nextField()) {
      ...
      switch (field) {
        ...
        case 3: {
          const m = InputDescriptor.initialize();
          reader.readMessage(m, InputDescriptor._readMessage);
          msg.inputDescriptors.push(m);
          break;
        }
        case 5: {
          const m = InputValue.initialize();
  ...
  _readMessageJSON: function (msg: Step, json: any): Step {
    ...
    const _inputDescriptors = json.inputDescriptors ?? json.input_descriptors;
    if (_inputDescriptors) {
      for (const item of _inputDescriptors) {
        const m = InputDescriptor.initialize();
        InputDescriptor._readMessageJSON(m, item);
        msg.inputDescriptors.push(m);
      }
    }
    const _answers = json.answers;
